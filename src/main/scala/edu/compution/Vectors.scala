...
trait Vector extends Serializable  with Writable {
   ...
   */
  private[edu] def toBreeze: BV[Double]

  /**
   * Gets the value of the ith element.
   * @param i index
   */
  private[edu] def apply(i: Int): Double = toBreeze(i)
}
...
object Vectors {
   ...
   */
  private[edu] def fromBreeze(v: BDV[Double]): DenseVector = {
   ...
   * Creates a vector instance from a breeze vector.
   */
  private[edu] def fromBreeze(v: BSV[Double]): SparseVector = {
...
class DenseVector extends Vector  {
  ...
  private[edu]  def toBreeze: BV[Double] = new BDV[Double](values)
  ...
  private[edu] def print(length: Int = size): String ={
    if (length >= size){
      toArray.mkString(",")
    }else {
...
class SparseVector extends Vector {
  ...
  private[edu]  def toBreeze: BV[Double] = new BSV[Double](indices, values, size)
